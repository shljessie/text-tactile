...
export const SonicTilesVideo = () => {
 ...
 let keyOptions;
  ...
  let [genCounts, setgenCounts ] = useState(0)
...
const stopLoadingSound = () => {
  ...
  const fetchImageName = async (imageURL) => {
    ...
    let customPrompt = `
        ...
        The title should be one of the following: Tree, Dog, Frisbee
  ...
  const generateImage = async (index, isRegeneration = false) => {
    ...
    try {
      ...
      if (voiceText) {
        ...
        if(correctCall) {
        setgenCounts(genCounts +1); 
        console.log('genCounts',genCounts);
        const response = await openai.createImage({
          prompt: ` You are an Illustrator. Your style is simple and minimalistic. Create ONLY ONE
           ${voiceText}
           There should be NO TEXT in the image. There should be NO BACKGROUND.
          `,
          n: 1,
          });
        ...
        const imageObjects = response.data.data.map(img => ({
          prompt: voiceText,
          name: '',
          url: url_list[genCounts],
          image_nbg: url_list[genCounts],
          descriptions: '',
          canvas_descriptions: '',
